...
4. **Make dependency direction explicit:** Identify upstream inputs, owned state, downstream consumers, external dependencies, and cross-module integration points. A module may depend only on contracts it names. Every cross-module contract you consume must be cited by its frozen registry ID (`C-xx`) from §6 of `03-tier1-epic.md` — cite the newest ID, never a superseded one. Do not restate the contract's shape from memory; read the registry entry.
...
8. **Keep design honest:** Do not claim a proposed interface already exists. Label it as proposed and identify the implementation seam that must establish it. A proposed contract is not a dependency that execution may assume is already integrated. This extends across modules: never assert another module's contract state unless that module's own artifact carries the same `C-xx`. A ruling applied to a peer module and merely assumed true of yours is a contract you do not have — cite the registry entry or raise it as an unresolved decision.
...
7. Preflight the output: module IDs and filenames are unique; all interface references resolve; every relevant invariant, NFR, and risk is traced; every consumed `C-xx` citation matches the frozen registry entry; and every proposed decision is visibly distinct from repository evidence.
   - **Cross-module agreement is not yours to verify.** A per-module architect cannot see peer modules. Scope your preflight to your own module's citations; the PM and the Plan Challenger own the cross-module check. If you cannot resolve a peer's contract state, report it as an unresolved decision rather than asserting agreement.
