...
The lightweight structure below is **not the draft** — it is a digest you extract _from_ the already-full file, used only as compact input for the Phase 4 reviewer prompts (Sections 2 and 3 in `adversarial-review.md`), so the CTO/Eng reviewer isn't handed the entire multi-page document per call.
